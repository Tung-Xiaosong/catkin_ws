		...
		if(read(dev_handler_, &recv_frames_[0], sizeof(recv_frames_[0])) >= 0)
		{
			for(int j=0;j<1;j++)
			{
				...
				switch (recv_frames_[0].can_id)
				{
					...
					case 0x98C4E8EF:
					{
						ulmsg.front_left = (unsigned short)((recv_frames_[0].data[1] & 0x0f) << 8 | recv_frames_[0].data[0]);
						ulmsg.left_front = (unsigned short)(recv_frames_[0].data[2]  << 4 | ((recv_frames_[0].data[1] & 0xf0) >> 4));
						ulmsg.left_rear= (unsigned short)((recv_frames_[0].data[4] & 0x0f) << 8 | recv_frames_[0].data[3]);				
						ulmsg.rear_left= (unsigned short)(recv_frames_[0].data[5]  << 4 | ((recv_frames_[0].data[4] & 0xf0) >> 4));
						break;
					}

					case 0x98C4E9EF:
					{
						ulmsg.rear_right = (unsigned short)(recv_frames_[0].data[5]  << 4 | ((recv_frames_[0].data[4] & 0xf0) >> 4));
						ulmsg.right_rear = (unsigned short)((recv_frames_[0].data[4] & 0x0f) << 8 | recv_frames_[0].data[3]);
						ulmsg.right_front = (unsigned short)(recv_frames_[0].data[2]  << 4 | ((recv_frames_[0].data[1] & 0xf0) >> 4));
						ulmsg.front_right = (unsigned short)((recv_frames_[0].data[1] & 0x0f) << 8 | recv_frames_[0].data[0]);
						...
						scan.ranges[1] = ((double)ulmsg.rear_right / 1000 > 0.5 || ulmsg.rear_right == 200) ? std::numeric_limits<float>::infinity() : (double)ulmsg.rear_right / 1000 + 0.62;
						scan.ranges[5] = ((double)ulmsg.right_rear / 1000 > 0.5 || ulmsg.right_rear == 200) ? std::numeric_limits<float>::infinity() : (double)ulmsg.right_rear / 1000 + 0.41;
						scan.ranges[12] = ((double)ulmsg.right_front / 1000 > 0.5 || ulmsg.right_front == 200) ? std::numeric_limits<float>::infinity() : (double)ulmsg.right_front / 1000 + 0.36;
						scan.ranges[17] = ((double)ulmsg.front_right / 1000 > 0.5 || ulmsg.front_right == 200) ? std::numeric_limits<float>::infinity() : (double)ulmsg.front_right / 1000 + 0.68;

						scan.ranges[19] = ((double)ulmsg.front_left / 1000 > 0.5 || ulmsg.front_left == 200) ? std::numeric_limits<float>::infinity() : (double)ulmsg.front_left / 1000 + 0.68;
						scan.ranges[24] = ((double)ulmsg.left_front / 1000 > 0.5 || ulmsg.left_front == 200) ? std::numeric_limits<float>::infinity() : (double)ulmsg.left_front / 1000 + 0.36;                                                                                                                                                       
						scan.ranges[30] = ((double)ulmsg.left_rear / 1000 > 0.5 || ulmsg.left_rear == 200) ? std::numeric_limits<float>::infinity() : (double)ulmsg.left_rear / 1000 + 0.41;						
						scan.ranges[34] = ((double)ulmsg.rear_left / 1000 > 0.5 || ulmsg.rear_left == 200) ? std::numeric_limits<float>::infinity() : (double)ulmsg.rear_left / 1000+ 0.62;
...
void CanControl::odomPub(float velocity,float steering)
{
	...
	double vx = velocity;
	double vth = vx * tan(steering) / 65;//106
...
//	th += delta_th;
	...
	x_mid = x + 0.325 * cos(th);
	y_mid = y + 0.325 * sin(th);
